...
const createBook = async (req, res) => {
    try {
        ...
        if (!validCategory.test(category)) {
            return res.status(400).send({ status: false, message: "plz enter valid Category" });
        }
...
const getBooks = async (req, res) => {
    ...
    if (queryData.length == 0) {
        let booksList = await bookModel.find({ isDeleted: false }).select({ _id: 1, title: 1, excerpt: 1, userId: 1, category: 1, releasedAt: 1, reviews: 1 })
    ...
    let booksList = await bookModel.find(queryData).select({ _id: 1, title: 1, excerpt: 1, userId: 1, category: 1, releasedAt: 1, reviews: 1 })
    if (booksList.length == 0) {
        return res.status(404).send({ status: false, message: "Data not Found" })
    }
    return res.status(200).send({ status: true, data: booksList })
